(function() {
	...
	var tadayData = myDate.getFullYear() + '-' + (myDate.getMonth() + 1) + '-' + myDate.getDate();
	...
	var Upload = {
		showCount: 0,
		uploadTpl: '<div class="edui-image-upload%%" style="display: none">' +
			'<span class="edui-image-icon"></span>' +
			'<form class="edui-image-form" method="post" enctype="multipart/form-data" target="up">' +
			'<input style=\"filter: alpha(opacity=0);\" class="edui-image-file" type="file" hidefocus name="upfile" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
			'</form></div>' +
			
			'<div id="ossfuwenbenId">' +
			'<form name=theform hidden>' +
			'<input type="radio" name="myradio" value="local_name" checked=true/>上传文件名字保持本地文件名字' +
			'<input type="radio" name="myradio" value="random_name"/>上传文件名字是随机文件名' +
			'<br/>' +
			'上传到指定目录:<input type="text" id="dirname" value="' + imgPathEditorOss + '" placeholder="如果不填，默认是上传到根目录" size=100>' +
			'</form>' +
			'<h4>您所选择的文件列表：</h4><img id="imgPreview" style="max-width: 150px;"/>' +
			'<div id="ossfile">' +
		...
		init: function(editor, $w) {
			var me = this;
			$(".edui-btn-primary").css("display", "none");
			...
			/*
			            //获取ID
			            if(typeof(editorImageId)!="undefined"){
...
$(".edui-modal-footer .edui-btn-primary").css("display", "none");
/*document.getElementById("postfiles").onclick = postfiles();
function postfiles(){
	$(".edui-modal-footer .edui-btn-primary").css("display","none");
	//testpostfiles();
}
...
function testpostfiles(){
	...
	var elem = document.getElementsByClassName("pic_list")[0].children[0].innerHTML = "<br>上传到OSS成功 <br>"
	var bool = true;
	for(var i = 0; i < piclist.length; i++) {
		if(piclist[i].children[0].innerHTML == "<br>上传到OSS成功 <br>") {
			$(".edui-modal-footer .edui-btn-primary").css("display","none");
		} else {
			bool = false;
		}

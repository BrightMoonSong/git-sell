...
function MainCtrl($rootScope, $scope, $timeout, constMessageCode, ngDialog, constManagerLocation) {
  ...
  $rootScope.showPrompt = function(promptMessage) {
		// 2秒内再次调用该方法，不执行
    $timeout(function() {
      $scope.showPromptStaus = false;
    }, 2000);
    if ($scope.showPromptStaus) {
      return 0;
    }
    $scope.showPromptStaus = true;
  ...
  //成功提示，相当于alert
  $rootScope.showAlert = function(alertMessage, time) {
    var dialog = ngDialog.open({
      template: '<h3>提示信息</h3>' +
        '<p>' + alertMessage + '</p>' +

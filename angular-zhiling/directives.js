	...
	return {
		link: function(scope, element) {
			var listener = function(event, toState, toParams, fromState, fromParams) {
				...
				if(toState.data && toState.data.pageTitle) title = '药到了 | ' + toState.data.pageTitle;
				$timeout(function() {
					element.text(title);
				});
			};
...
//代售商品说明书数据处理
function explainDataTypeHtml(html) {
	var str = html.split('<div style="display: none;">');
	if(str.length == 1) {
		return 1;
	} else {
		try {
			str.removeByValue("");
			var sssss = str[str.length - 1];
			var fds = sssss.split('</div>');
			fds.removeByValue('')
			return fds[0];
		} catch(e) {
			return 1;
			...
			$scope.$parent.inner()
				.then(function(res) {
					...
					var marker = new AMap.Marker({
						map: map,
						bubble: true,
						content: '<div><div class="marker-route marker-marker-bus-from"></div><div id="panel" style="width:0;height:0;"></div></div>' //自定义点标记覆盖物内容,
					});

					if(res != 1) {
						if($scope.$parent.chainEntity) {
							if($scope.$parent.chainEntity && $scope.$parent.chainEntity.lon && $scope.$parent.chainEntity.lat) {
						...
						marker = new AMap.Marker({
							...
							position: [$scope.$parent.chainEntity.lon, $scope.$parent.chainEntity.lat],
					...
					AMap.event.addDomListener(document.getElementById('query'), 'click', function() {
						...
						AMap.service(["AMap.PlaceSearch"], function() {
							...
							placeSearch.search(cityName, function(status, data) {
								if(status !== 'complete') return;
								var pois = data.poiList.pois;
								//脏值检查
								$scope.$apply(function() {
									document.getElementById("lnglat").value = pois[0].location.lat;
									document.getElementById("lngla").value = pois[0].location.lng;
									$scope.$parent.chainEntity.lat = pois[0].location.lat;
									$scope.$parent.chainEntity.lon = pois[0].location.lng;
								})
							});
						});
						clickEventListener();

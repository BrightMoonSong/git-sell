angular
	.module('managerApp')
	.factory('OrdersService', function($http, $q, constMapiLocation) {
		...
		return {
			//获取订单列表
			find: function(parmValue, storeId, status, orderStatus, pageSize, pageNo) {
				var defer = $q.defer();
				var url = baseUrl + "/findinfos?pageSize=" + pageSize + "&pageNo=" + pageNo + "&status=" + status;
				if(storeId) {
					url += "&storeId=" + storeId;
				}
				if(parmValue) {

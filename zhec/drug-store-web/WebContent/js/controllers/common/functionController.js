function functionController($rootScope, $scope, $http, $q, constPageSize, ngDialog, functionService, constManagerLocation,$interval, $state) {
  /**
   * 获取导航栏列表
   */
  $scope.find = function() {
  ...
  $scope.getnotice = function() {
    ...
    functionService
      .getnotice($scope.userToken)
      .then(function(result) {
          if (result.code === 0) {
            try {
              if (result.data.hasNewOrder) {
                // 播放提示有新订单的声音
                playSound();
              }
            } catch (e) {
  ...
  var timeout_upd = $interval($scope.getnotice, 10000);
  //登出   清除缓存

function OrdersController($scope, OrdersService, $rootScope, $q, ngDialog, $interval) {
	...
	$scope.find = function(currentPageNo, currentPaseSize) {
		...
		if($scope.orderNameSearch) { //查询关键词
			parmValue = $scope.orderNameSearch;
		} else {
			parmValue = '';
		}
		OrdersService
			.find(parmValue, storeId, status, $scope.orderStatusSearch, currentPaseSize, currentPageNo)
			.then(
				function(result) {
	...
	$scope.intervalOrder = $interval(function() {
		var href = window.location.href;
		if(isContains(href, 'orderslist')) {
			$scope.loadData();
		} else {
			$interval.cancel($scope.intervalOrder);
		}
	}, 10000);
...
//详情
function OrdersInfoModalController($scope, OrdersService, $rootScope, ngDialog) {
	//初始化
...
//订单评价的弹窗controller
function OrdersAppraisalModalController($scope, OrdersService, $rootScope) {
...
//详情以外的按钮的弹窗
function OrdersModalConfirmController($scope, OrdersService, $rootScope) {
	$scope.remark = '';
	//保存

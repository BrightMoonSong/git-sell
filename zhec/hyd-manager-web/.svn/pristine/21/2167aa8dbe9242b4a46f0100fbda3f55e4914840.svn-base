angular
	.module("managerApp")
	.factory("ShipperOkService", function($q, $http, constMapiLocation) {
		var baseUrl = constMapiLocation + '/shipper';
		return {
			//待审核列表
			find: function(realName, phone, startTime, pageNo, pageSize) {
				var defer = $q.defer();
				var url = baseUrl + '/shippers?pageNo=' + pageNo + '&pageSize=' + pageSize;
				if(realName) {
			...
			//修改
			edit: function(id,settlementMethod) {
				var defer = $q.defer();
				var url = baseUrl + '/updateSettlementMethod/' + id + '?settlementMethod=' + settlementMethod;
				$http({
					method: 'put',
					url: url
				}).success(function(data) {

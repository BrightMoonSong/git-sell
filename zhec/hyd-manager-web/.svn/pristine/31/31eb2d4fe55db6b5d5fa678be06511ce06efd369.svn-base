function ExceptionController($scope, $q, ExceptionService, constPageSize, ngDialog, goodsReminder) {
	$scope.WdatePicker = {};
	...
	$scope.find = function(currentPageNo, currentPaseSize) {
		...
		ExceptionService
			.find(currentPageNo, currentPaseSize, requestMethod, interfaceName, phone, $scope.startTimeStr, $scope.endTimeStr, platform, requestUri,code)
	...
	$scope.openModal = function() {
		$scope.dialog = ngDialog.open({
			template: 'views/log/ExceptionFormModel.html',
			className: 'ngdialog-theme-default',
			controller: 'ExceptionFormModelController',
			scope: $scope,
			width: 850
		})
	};
	...
	$scope.openModalDetail = function(id) {
		...
		$scope.dialog1 = ngDialog.open({
			template: 'views/log/ExceptionDetailFormModel.html',
			className: 'ngdialog-theme-default',
			controller: 'ExceptionDetailFormModelController',
			scope: $scope,
			width: 850
		})
	};
...
//批量删除弹窗操作
function ExceptionDetailFormModelController($scope, ExceptionService) {
	//详情初始化
...
function ExceptionFormModelController($scope, $rootScope, ExceptionService) {
	//批量删除
...
angular
	...
	.controller("ExceptionDetailFormModelController", ExceptionDetailFormModelController)
	.controller("ExceptionFormModelController", ExceptionFormModelController)

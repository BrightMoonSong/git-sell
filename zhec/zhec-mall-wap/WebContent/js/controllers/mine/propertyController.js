...
function propertyController($rootScope, $scope, propertyservice, $q) {
	...
	$scope.getmemberspentdate = function(memberID) {
		...
		propertyservice
			...
			.then(function(result) {
				...
				console.log($scope.spentScoreList)
				defer.resolve(result);
			}, function(result) {
				defer.reject(result);
			});
	...
	$scope.replace = function() {
		window.location.replace('/index.html#/info/propertyDetail')
	}
	$scope.replace1 = function() {
		window.location.replace('/index.html#/info/property')
	}

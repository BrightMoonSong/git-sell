function SearchListController($scope, $q, SearchListService, CommonService, constPageSize, $stateParams) {

	$scope.crruntSelBrand = ''; //当前选中的品牌
	$scope.crruntselCategory = ''; //当前选中的分类
	$scope.crruntSelFilter = []; //当前选中的过滤属性
	$scope.crruntSelMore = []; //当前显示更多的数据
	$scope.isGoodsListEmpty = false;

	$scope.paramsKeyWord = decodeURI($stateParams.paramsKeyWord);
	...
	$scope.keyWordShow = $scope.paramsKeyWord;
	...
	$scope.backUrl = getCookie('saveUrl');
	...
	$scope.find = function(currentPageNo) {
		var defer = $q.defer();
		SearchListService
			.findGoodsList($scope.paramsKeyWord, 2, $scope.paramsCategoryId, currentPageNo, $scope.paramsOrderid, $scope.paramsInstock, $scope.paramsBrandId, $scope.paramsFilterId)
			.then(function(result) {
					...
					if (result.goodsInfo.goods != null && result.goodsInfo.goods != undefined && result.goodsInfo.goods != '' && result.goodsInfo.goods != []) {
						...
						//已经选择的分类
						$scope.selCategory = result.selectedInfo.selCategory;
						$scope.selCategoryStr = JSON.stringify($scope.selCategory);
						if (!$scope.selCategoryStr) {
							for (key in $scope.selCategory) {
								$scope.crruntselCategory = key + ":" + $scope.selCategory[key];
							}
						} else {
							$scope.crruntselCategory = '';
						}
						//已选择的过滤属性
						...
						//未选择的分类
						$scope.displayCategory = result.displayInfo.cate;
						$scope.displayCategoryStr = JSON.stringify($scope.displayCategory);
						console.log($scope.displayCategory);
						//未选择的过滤属性
	...
	$scope.selectCategory = function(category) {
		$scope.crruntselCategory = category;
		console.log($scope.crruntselCategory);
	};

	//判断分类是否是选中状态
	$scope.isCategorySelected = function(category) {
		return category === $scope.crruntselCategory;
	};
	...
	$scope.showMore = function(key) {
		...
		console.log($scope.crruntSelMore);
	};
	...
	$scope.commitFiter = function() {
		...
		window.open(constWapLocation + "/index.html#/info/search-" + $scope.paramsKeyWord + "-" + $scope.paramsCategoryId + "-" + $scope.paramsOrderid + "-" + $scope.paramsInstock + "-" + brand + "-" + filter, "_self");
	};

	//返回上一页
	$scope.goBack = function() {
		if ($scope.backUrl) {
			window.open($scope.backUrl, "_self");
		} else {
			window.open(constWapLocation + '/index.html', "_self");
		}
	};

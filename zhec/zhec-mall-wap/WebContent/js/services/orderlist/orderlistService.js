 ...
 */
app
  .factory('orderlistService',function($http,$q){

    return {
      //查询数据方法
      find:function(memberId,pageSize,pageNo,orderState,startTime,endTime, parmValue){
        var defer = $q.defer();
        var baseUrl= constWapLapiLocation + '/member/findorders';
      ...
      putOrder:function(order){
        var defer = $q.defer();
        var url= constWapLapiLocation + '/member'+ '/cancel';
        $http({
          method:'put',
          url:url,
          data:order
      ...
      putTotalOrder:function(id,memberId){
        var defer = $q.defer();
        var url= constWapLapiLocation + '/member/cancelparentorder?orderId='+id+"&memberId="+memberId;
        $http({
          method:'put',
          url:url
        }).success(function(data){

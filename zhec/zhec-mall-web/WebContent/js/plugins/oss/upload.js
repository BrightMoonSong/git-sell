...
function loadXMLDoc() {
	...
	xmlhttp.open("GET", ConstBaseLocation + "/oss/getsign", true);
	xmlhttp.send();
}
...
var uploader = new plupload.Uploader({
	...
	filters: {
		...
		max_file_size: '2048kb', //最大只能上传 2048kb 的文件
	...
	init: {
		...
		FilesAdded: function(up, files) {
			...
			if(filename.split("/")[2] == 'submitBill' || filename.split("/")[2] == 'prescription' || filename.split("/")[1] == 'refund') {
				var pListLength = files.length;
				var fileslength = uploader.files.length;
				var pListLengthDom = $(".pic_list").length;
				var total = pListLengthDom + pListLength;
				//uploader.files.push(file);
				if(pListLength > 5 || total > 5) {
					if(typeof imgPathAfterSales != "undefined") {
						imgPathAfterSales = '';
					}
					if(typeof imgPathReturn != "undefined") {
						imgPathReturn = [];
					}
					if(typeof imgPath != "undefined") {	//处方
						imgPath = [];
					}
					document.getElementById('ossfile').innerHTML = '';
					var t = setTimeout(uploader.init, 500);
					promptBox("最多上传5张!请重新选择");
					uploader.files = new Array();
					files = new Array();
			...
			plupload.each(files, function(file) {
				...
				previewImage(file, function(imgsrc) {
					document.getElementById('ossfile').innerHTML +=
						...
						'<b></b>' +
						'<img src="../../../images/icon1_03.png" class="pic_delete" data-val=' + file.id +
						' style="position: absolute;z-index: 9999;"><br/>' +
						'<a><img class="listview" style="max-width: 100px;" src="' + imgsrc + '" name="' + file.name + '" /></a>' +
		...
		FileUploaded: function(up, file, info) {
			...
			if(info.status == 200) {
				...
				document.getElementById(file.id).getElementsByTagName('b')[0].innerHTML = '<img src="../../../images/duigou.png">';
				var imgaa = '<img src="';
				imgaa += aa;
				imgaa += '" _src = "';
				imgaa += aa;

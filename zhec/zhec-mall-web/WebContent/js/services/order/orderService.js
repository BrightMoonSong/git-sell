...
angular
	.module('orderApp')
	.factory('orderService', function($http, $q, constBaseLocation) {
		...
		return {
			...
			findsubmitproducts: function(orderSource, memberId) {
				var defer = $q.defer();
				var url = constBaseLocation + '/orders/findsubmitproducts?orderSource=' + orderSource + '&memberId=' + memberId;
				$http({
					method: 'get',
					url: url
				}).success(function(data) {
			...
			submit: function(res, submitType) {
				var defer = $q.defer();
				var url = constBaseLocation + '/orders/submit?submitType=' + submitType;
				$http({
					method: 'post',
					url: url,
					data: res
			...
			finddirectpurchaseproducts: function(orderSource, consultantId, productId, productCount, memberId) {
				var defer = $q.defer();
				var url = constBaseLocation + '/orders/finddirectpurchaseproducts?orderSource=' + orderSource + '&consultantId=' + consultantId + '&productId=' + productId + '&productCount=' + productCount + '&memberId=' + memberId;
				$http({
					method: 'get',
					url: url
				}).success(function(data) {
			...
			getlastinvoice: function(memberId) {
				var defer = $q.defer();
				var url = constBaseLocation + '/orders/getlastinvoice?memberId=' + memberId;
				$http({
					method: 'get',
					url: url
				}).success(function(data) {
			...
			addInvoice: function(res) {
				var defer = $q.defer();
				var url = constBaseLocation + '/orders/addInvoice';
				$http({
					method: 'post',
					url: url,
					data: res

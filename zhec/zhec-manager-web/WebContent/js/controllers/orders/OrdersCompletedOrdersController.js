 ...
 * 系统用户controller定义
 */
function OrdersCompletedOrdersController($scope, $q, constPageSize, OrdersCompletedOrdersService, ngDialog) {
	...
	$scope.find = function(currentPageNo, currentPaseSize) {
		...
		OrdersCompletedOrdersService.find(memberName, orderSn, minTime, maxTime, currentPaseSize, currentPageNo).then(
			function(result) {
 ...
 * 弹出详情页面的controller定义
 */
function OrdersAllFormModalController($scope, OrdersCompletedOrdersService) {
	$scope.initEntity = function() {
		$scope.currentTab = "ordersGoods.html";
		OrdersCompletedOrdersService
			.get($scope.dataId)
			.then(
				function(result) {
...
angular
	.module('managerApp')
	.controller('OrdersCompletedOrdersController', OrdersCompletedOrdersController)

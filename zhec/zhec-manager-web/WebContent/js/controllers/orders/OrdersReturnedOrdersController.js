 ...
 * 系统用户controller定义
 */
function OrdersReturnedOrdersController($scope, $q, $rootScope, constPageSize, OrdersCancelledOrdersService, ngDialog) {
	...
	$scope.find = function(currentPageNo, currentPaseSize) {
		...
		OrdersCancelledOrdersService.find(memberName, orderSn, minTime, maxTime, currentPaseSize, currentPageNo).then(
			function(result) {
 ...
 * 弹出详情页面的controller定义
 */
function OrdersAllFormModalController($scope, OrdersCancelledOrdersService) {
	$scope.initEntity = function() {
		$scope.currentTab = "ordersGoods.html";
		OrdersCancelledOrdersService
			.get($scope.dataId)
			.then(
				function(result) {
...
angular
	.module('managerApp')
	.controller('OrdersReturnedOrdersController', OrdersReturnedOrdersController)

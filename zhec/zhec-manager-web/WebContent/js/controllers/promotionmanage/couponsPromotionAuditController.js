 ...
 * 系统用户controller定义
 */
function couponsPromotionAuditController($scope, $q, constPageSize, couponsPromotionAuditService, ngDialog) {
	...
	$scope.find = function(currentPageNo, currentPaseSize) {
		...
		//列表
		couponsPromotionAuditService.find(parmValue, minTime, maxTime, currentPaseSize, currentPageNo).then(
			function(result) {
	...
	$scope.openModal = function(n, id) {
		...
		if(n == 1) {
			$scope.dialog = ngDialog.open({
				...
				controller: 'couponsGoodsPromotionAuditModalController',
				scope: $scope,
				width: 960
			});
		}
		...
		if(n == 3) {
			$scope.dialog = ngDialog.open({ //详情
				...
				className: 'ngdialog-theme-default',
				controller: 'couponsGoodsPromotionAuditDetailsModalController',
				scope: $scope,
				width: 1160
			});
		}
 ...
 * 弹出页面的controller定义
 */
function couponsGoodsPromotionAuditDetailsModalController($scope, couponsPromotionAuditService) {
	//根据平台类型
	$scope.ifshowtype = "";
	//详情
	$scope.details = function() {
		couponsPromotionAuditService
 ...
 */
function couponsGoodsPromotionAuditModalController($scope, couponsPromotionAuditService) {
	$scope.dataEntity = {};
	//保存
...
angular
	.module('managerApp')
	.controller('couponsPromotionAuditController', couponsPromotionAuditController)
	.controller('couponsGoodsPromotionAuditDetailsModalController', couponsGoodsPromotionAuditDetailsModalController)
	.controller('couponsGoodsPromotionAuditModalController', couponsGoodsPromotionAuditModalController)
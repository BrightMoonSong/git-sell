 ...
 * 系统用户controller定义
 */
function RefundAllController($rootScope, $scope, $q, constPageSize, RefundAllService, ngDialog) {
	...
	//findallstatus---查询退单状态信息
	$scope.first = function() {
		//查询退单状态信息  
		RefundAllService.findallstatus().then(
			function(result) {
	...
	$scope.find = function(currentPageNo, currentPaseSize) {
		...
		//列表
		RefundAllService.find(orderStatus, memberName, orderSn, minTime, maxTime, currentPaseSize, currentPageNo).then(
			function(result) {
 ...
 * 弹出页面的controller定义
 */
function RefundAllFormModalController($rootScope, $scope, RefundAllService) {
	$scope.data = {};
...
angular
	.module('managerApp')
	.controller('RefundAllController', RefundAllController)
